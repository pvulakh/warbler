...
RSpec.describe WarblesController, type: :controller do
  ...
  describe 'DELETE #destroy' do
    let!(:warble) { create(:warble) }

    before(:each) do
    # whenever the instance of WarbleController calls the method
    # current_user and returns the warbles author 
    # (the warble we created in the let!)
      allow(subject).to receive(:current_user).and_return(warble.author)
      # giving the request the correct params
      delete :destroy, params: { id: warble.id } 
    end
    
    it 'should delete the warble' do
      # exist is an active record method. just checks in the warbles table
      # if the specific id exists in the db.
      expect(Warble.exists?(warble.id)).to be(false)
    end

    it 'should redirect us to the warble index' do
      expect(response).to have_http_status(302)
      expect(response).to redirect_to(warbles_url)
    end
